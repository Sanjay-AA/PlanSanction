...
import { Building2, ShieldCheck, Mail, Phone, MapPin, User, Save } from 'lucide-react';
...
export const OfficerProfile: React.FC = () => {
  ...
  const [designation, setDesignation] = useState(currentUser.designation || 'Town Planning Officer');
    ...
    <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <div className="text-xs text-slate-500 flex items-center gap-2 mt-0.5">
              ...
              <span>•</span>
              <span>{currentUser.region} Municipal Planning Authority</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-6 sm:p-8 rounded-xl border border-slate-200 shadow-sm">
        <div className="border-b border-slate-200 pb-3 mb-6">
          <h3 className="text-base font-bold text-slate-900">Planning Officer Jurisdiction & Credentials</h3>
          <p className="text-xs text-slate-500">
            Designated municipal building plan scrutiny authority for Directorate of Town and Country Planning (DTCP).
          </p>
        </div>
        ...
        {savedSuccess && (
          <div className="p-3 mb-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg text-xs font-semibold flex items-center space-x-2">
            ...
            <span>Officer credentials updated successfully.</span>
          </div>
        )}
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Official Government Email (Read-Only)
              </label>
              <div className="relative">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Taluk Jurisdiction (Assigned by District Admin)
              </label>
              <div className="relative">
                <MapPin className="w-4 h-4 text-slate-400 absolute left-3 top-2.5" />
                <input
                  type="text"
                  value={`${currentUser.region} Municipality, Namakkal District`}
                  readOnly
